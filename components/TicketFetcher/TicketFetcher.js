...
class HttpTicketRequest {
   ...
   *q
   * @param {String} token Basic Authentication Token
   */
  constructor(token) {
    this.headers = new fetchHeaders()
    this.headers.append('Authorization', 'Basic ' + token)
    this.headers.append('method', 'GET')
   }
  ...
  async fetchAllTickets() {
    const perPage = 25
    let url = `https://zccsiddheshamrale.zendesk.com/api/v2/tickets.json?per_page=${perPage}`
    let page = 0
    return async () => {
      if (url != null) {
  ...
  async fetchTicketById(ticketId) {
    let url = `https://zccsiddheshamrale.zendesk.com/api/v2/tickets/${ticketId}.json`
    let apiResponse = await this.fetchRequest(url)
